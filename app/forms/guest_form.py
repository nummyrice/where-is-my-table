...
class NewGuestForm(FlaskForm):
    ...
    phone_number = StringField('phone_number', validators=[Length(min=10, max=11), DataRequired(), phone_num_exists])
...
class UpdateGuestForm(FlaskForm):
    ...
    phone_number = StringField('phone_number', validators=[Optional(strip_whitespace=True), Length(min=10, max=11), phone_num_exists])
